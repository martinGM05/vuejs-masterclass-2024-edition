...
const { handleLoginForm, handleServerError, serverError, realtimeErrors } = useFormErrors()
...
const signin = async () => {
  const realtimeErrors = await handleLoginForm(formData.value)

  if (realtimeErrors.email.length > 0 || realtimeErrors.password.length > 0) return
    ...
    <Card class="max-w-sm w-full mx-auto">
      ...
      <CardContent>
        ...
        <form class="grid gap-4" @submit.prevent="signin">
          ...
          <ul class="text-sm text-left text-red-500" v-if="realtimeErrors?.email.length > 0">
          ...
          <ul class="text-sm text-left text-red-500" v-if="realtimeErrors?.password.length > 0">
